...
describe("generator-norgate-av:typescript:skipInstall", () => {
    ...
    it("should create the correct .all-contributorsrc", () => {
        assert.fileContent(".all-contributorsrc", `"projectName": "${name}"`);
    });
...
describe("generator-norgate-av:typescript:install", () => {
    let result: RunResult<AppGenerator>;
    ...
    beforeEach<Promise<RunResult<AppGenerator>>>(async () => {
        result = await helpers.run<AppGenerator>(generator).withAnswers({
            type: "typescript",
            name,
            description,
            git: false,
